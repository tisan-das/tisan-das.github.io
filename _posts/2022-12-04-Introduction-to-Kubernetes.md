...
#### Deployments: 
Higher abstraction to replica sets, provides a mechanism to change to desired state in a controlled way through replica sets.
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
### CronJob:  
CronJob creates Jobs on a repeating schedule. One CronJob object is like one line of a crontab (cron table) file. It runs a job periodically on a given schedule, written in Cron format.
```yaml
apiVersion: batch/v1
kind: CronJob
metadata:
