...
Docker network also provides an support for network alias on the context of a particular network, which is resolved at run-time. This particular functionality can be used as load-balancing service. Example:
...
More on the Docker network can be found at https://docs.docker.com/network/
